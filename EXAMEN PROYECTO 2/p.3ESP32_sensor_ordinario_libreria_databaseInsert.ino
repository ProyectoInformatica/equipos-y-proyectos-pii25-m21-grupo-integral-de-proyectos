...
// Libreria del sensor ordinario.
// Si vuestra libreria tiene otro nombre de archivo .h, cambia solo esta linea.
...
// Sensor ordinario mediante libreria RX/TX.
// La libreria propuesta ofrece:
//   inicializarSensor(pin_rx, pin_tx)       -> void
//   comprobarDatosDisponibles()            -> bool
//   leerDatosOrdinarios()                  -> datos con: int + char[]
//
// Pines libres del ESP32 usados para la comunicacion con el sensor.
// Cambialos si vuestro montaje usa otros pines.
#define PIN_SENSOR_ORDINARIO_RX   16
#define PIN_SENSOR_ORDINARIO_TX   17
#define ID_TIPO_SENSOR_ORDINARIO   8
...
// ============================================================
//  FUNCION: escapar texto para SQL
//  Evita errores si un texto alfanumerico contiene comillas.
// ============================================================
void escaparSQL(const char* entrada, char* salida, size_t tamSalida) {
  if (entrada == NULL || tamSalida == 0) {
    if (tamSalida > 0) salida[0] = '\0';
    return;
  }

  size_t j = 0;

  for (size_t i = 0; entrada[i] != '\0' && j < tamSalida - 1; i++) {
    char c = entrada[i];

    if ((c == '\'' || c == '\\') && j < tamSalida - 2) {
      salida[j++] = '\\';
      salida[j++] = c;
    } else {
      salida[j++] = c;
...
// ============================================================
//  FUNCION: databaseInsert
//  Recibe datos numericos y alfanumericos generados por sensores
//  y los inserta en la tabla Sensores.
//
//  IMPORTANTE:
//  La tabla Sensores debe tener la columna:
//    Valor_Alfanumerico VARCHAR(100) NULL
// ============================================================
bool databaseInsert(int id_tipo, float valorNumerico, const char* valorAlfanumerico) {
  if (!conectarDB()) {
    Serial.println("  ERROR: no hay conexion con MySQL.");
    return false;
  }

  char textoSeguro[120];
  escaparSQL(valorAlfanumerico, textoSeguro, sizeof(textoSeguro));

  char sql[500];

  snprintf(sql, sizeof(sql),
    "INSERT INTO `%s`.`Sensores` "
    "(`Valor`,`Valor_Alfanumerico`,`Fecha`,`Id_Tipo`,`Activo`) "
    "VALUES (%.2f,'%s',NOW(),%d,1);",
    DB_NAME,
    valorNumerico,
    textoSeguro,
  ...
  MySQL_Cursor cursor(&dbConn);
  bool ok = cursor.execute(sql);
  cursor.close();

  if (ok) {
    Serial.print("  Sensor guardado -> Id_Tipo: ");
    Serial.print(id_tipo);
    Serial.print(" | Valor: ");
    Serial.print(valorNumerico);
    Serial.print(" | Texto: ");
    Serial.println(textoSeguro);
  } else {
    Serial.println("  ERROR al insertar sensor en BD.");
  }
...
// ============================================================
//  ADAPTADOR DEL SENSOR ORDINARIO
//
//  La libreria indicada devuelve dos datos:
//    - dato numerico: int
//    - dato alfanumerico: char[]
//
//  En C++/Arduino, normalmente ese par de datos se entrega
//  mediante una estructura. En este proyecto se espera que la
//  libreria devuelva un tipo llamado DatosOrdinarios con estos
//  campos:
//      datos.valorNumerico
//      datos.valorAlfanumerico
//
//  Si vuestra libreria usa otros nombres, solo cambia las 2 lineas
//  marcadas dentro de leerSensorOrdinarioYGuardar().
// ============================================================

// ============================================================
//  FUNCION: leer sensor ordinario y guardar en BD
//  Usa la libreria del sensor ordinario:
//    comprobarDatosDisponibles()
//    leerDatosOrdinarios()
//  Despues inserta en MySQL usando databaseInsert().
// ============================================================
void leerSensorOrdinarioYGuardar() {
  ...
  // Si vuestra libreria usa otros nombres de campos, cambia solo estas lineas.
  int valorNumerico = datos.valorNumerico;
  const char* valorAlfanumerico = datos.valorAlfanumerico;

  if (valorAlfanumerico == NULL || strlen(valorAlfanumerico) == 0) {
    valorAlfanumerico = "SENSOR_ORDINARIO_SIN_TEXTO";
  }

  Serial.print("  Sensor ordinario -> Valor numerico: ");
  Serial.print(valorNumerico);
  Serial.print(" | Valor alfanumerico: ");
  Serial.println(valorAlfanumerico);

  bool insertado = databaseInsert(
    ID_TIPO_SENSOR_ORDINARIO,
    (float)valorNumerico,
    valorAlfanumerico
  );

  if (insertado) {
    Serial.println("  Sensor ordinario guardado correctamente en BD.");
  } else {
    Serial.println("  ERROR: no se pudo guardar el sensor ordinario en BD.");
  }
}
...
void guardarSensor(int id_tipo, float valor, float umbral,
                   ...
                   const char* valorAlfanumerico) {

  bool insertado = databaseInsert(id_tipo, valor, valorAlfanumerico);

  if (!insertado) {
    Serial.println("  No se pudo guardar el sensor.");
    return;
  }

  if (superaUmbral) {
    char tituloSeguro[120];
    char textoSeguro[120];

    escaparSQL(tituloAlerta, tituloSeguro, sizeof(tituloSeguro));
    escaparSQL(valorAlfanumerico, textoSeguro, sizeof(textoSeguro));

    char sql[500];

    snprintf(sql, sizeof(sql),
      "INSERT INTO `%s`.`Notificaciones` "
      "(`Fecha`,`Dato`,`Dato_Alfanumerico`,`Titulo`,`Id_Sensor`,`Activo`) "
      "VALUES (NOW(),%.2f,'%s','%s',LAST_INSERT_ID(),1);",
      DB_NAME,
      valor,
      textoSeguro,
      tituloSeguro
...
void setup() {
  ...
  // Inicializacion del sensor ordinario mediante la libreria proporcionada.
  inicializarSensor(PIN_SENSOR_ORDINARIO_RX, PIN_SENSOR_ORDINARIO_TX);
...
void loop() {
  ...
  if (ahora - ultimoSensores >= INTERVALO_SENSORES) {
    ...
    if (!isnan(temp)) {
      ...
      guardarSensor(1, temp, umbralTemperatura, "Temperatura alta", alerta, "TEMPERATURA_DHT11");
    }
    // Humedad (Id_Tipo=2)
    if (!isnan(hum)) {
      bool alerta = hum > umbralHumedad;
      guardarSensor(2, hum, umbralHumedad, "Humedad critica", alerta, "HUMEDAD_DHT11");
    }
    // Luminosidad escalada a 0-1000 (Id_Tipo=3)
    float ldrEscalado = map(valorLDR, 0, 4095, 0, 1000);
    bool alertaLuz = ldrEscalado < 100;
    guardarSensor(3, ldrEscalado, 100, "Luminosidad baja", alertaLuz, lucesEncendidas ? "LUCES_ON" : "LUCES_OFF");

    // Gas / Calidad de aire (Id_Tipo=4)
    bool alertaGas = gas > (int)umbralGas;
    guardarSensor(4, (float)gas, umbralGas, "Gas elevado detectado", alertaGas, alertaGas ? "GAS_ALTO" : "GAS_NORMAL");

    // Viento (Id_Tipo=5) — siempre guarda, alerta si supera 50 km/h
    bool alertaViento = viento > 50.0f;
    guardarSensor(5, viento, 50.0f, "Viento peligroso", alertaViento, alertaViento ? "VIENTO_ALTO" : "VIENTO_NORMAL");

    // Sensor ordinario (Id_Tipo=8)
    // Lee el sensor, clasifica su estado y lo guarda con databaseInsert().
  ...
  if (ahora - ultimoSerial >= INTERVALO_SERIAL) {
    ...
    Serial.print(" | Viento: "); Serial.print(viento, 1); Serial.print(" km/h");
    Serial.print(" | Sensor ordinario datos: ");
    Serial.println(comprobarDatosDisponibles() ? "DISPONIBLES" : "NO DISPONIBLES");
    Serial.print("  WiFi: ");
