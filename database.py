...
_SENSOR_ALIASES: Dict[str, str] = {
    "temp":    "Temperatura",
    "hum":     "Humedad",
    "iaq":     "Gas",          # sensor MQ cubre calidad de aire
    "humo":    "Gas",          # mismo sensor MQ
    "luz":     "Luminosidad",
    "viento":  "Viento",
    "water":   "Flujo de Agua",
    "power":   "Potencia",
...
# Pulsometro
def add_pulsometro_data(frecuencia_cardiaca, presion_sistolica, presion_diastolica):
    conn = get_connection()
    try:
        cursor = conn.cursor()
        sql = """
            ...
            VALUES (%s, %s, %s, NOW(), 1)
        """
        valores = (frecuencia_cardiaca, presion_sistolica, presion_diastolica)
...
def get_pulsometro_data(limit=20):
    conn = get_connection()
    try:
        cursor = conn.cursor(dictionary=True)
        sql = f"""
...
def update_pulsometro_data(id_pulsometro, frecuencia_cardiaca, presion_sistolica, presion_diastolica):
    conn = get_connection()
    try:
        cursor = conn.cursor()
        sql = """
            UPDATE Pulsometro
            SET Frecuencia_Cardiaca = %s,
                Presion_Sistolica = %s,
                Presion_Diastolica = %s
            WHERE Id_Pulsometro = %s AND Activo = 1
        """
        valores = (frecuencia_cardiaca, presion_sistolica, presion_diastolica, id_pulsometro)
        cursor.execute(sql, valores)
        conn.commit()
