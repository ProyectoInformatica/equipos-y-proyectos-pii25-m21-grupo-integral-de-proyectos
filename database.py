...
def get_biordinario_data(limit: int = 24) -> List[Dict]:
    """
    Devuelve las últimas lecturas del sensor biordinario.
    Cada registro contiene el dato numérico (Valor) y el alfanumérico (Valor_Texto).

    Parámetros
    ----------
    limit : int  número máximo de registros (por defecto 24)

    Devuelve
    --------
    [{"valor": float, "texto": str, "hora": str}, ...]
    ...
    try:
        ...
        cursor.execute(
            ...
            SELECT s.Valor       AS valor,
                   s.Valor_Texto AS texto,
                   s.Fecha       AS hora
            FROM   Sensores s
            JOIN   Tipos    t ON s.Id_Tipo = t.Id_Tipos
            WHERE  t.Nombre = 'biordinario'
              AND  s.Activo = 1
