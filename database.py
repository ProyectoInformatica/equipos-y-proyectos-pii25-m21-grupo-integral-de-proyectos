...
def _get_barrera_id(ubicacion: str) -> int:
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        map_nombres = {"norte": "Entrada principal", "sur": "Salida trasera"}
        nombre_real = map_nombres.get(ubicacion.lower(), ubicacion)
...
def set_access_distance(barrera: str, distance_cm: float):
    add_sensor_data(f"Distancia - {barrera}", distance_cm, category="distancia")

def get_access_distance(barrera: str = None) -> Dict:
    if barrera:
        d = get_latest_sensor_value(f"Distancia - {barrera}", category="distancia")
        return {barrera: {"distance_cm": d if d > 0 else 500}}
    n = get_latest_sensor_value("Distancia - norte", category="distancia")
    s = get_latest_sensor_value("Distancia - sur", category="distancia")
    return {
        "norte": {"distance_cm": n if n > 0 else 500},
        "sur": {"distance_cm": s if s > 0 else 500}
    }
...
def get_access_state(barrera: str = None) -> Dict:
    if barrera:
        ...
        d = get_access_distance(barrera)[barrera]["distance_cm"]
        return {"barrera_abierta": (st == 'abierta'), "mensaje": f"BARRERA {st.upper()}", "distancia_detectada": d}
    else:
        res = {}
        for b in ["norte", "sur"]:
...
def get_access_log(limit: int = 20) -> List[Dict]:
    ...
    try:
        ...
        for r in rows:
            ...
            map_inv = {"Entrada principal": "norte", "Salida trasera": "sur"}
            r['barrera'] = map_inv.get(r['barrera'], r['barrera'])
            res.append(r)
        return res
    finally:
        conn.close()
