...
//      Id_Tipos 4 -> Gas
//      Id_Tipos 5 -> Viento
//    Sensor biordinario (tabla dedicada Sensores_Biordinarios):
//      ValorNumerico + ValorAlfanumerico (UART Serial2)
//    Barreras:
...
#define PIN_SENSOR_RX  16   // Serial2 RX — sensor biordinario
#define PIN_SENSOR_TX  17   // Serial2 TX — sensor biordinario
...
int datosOrdinarios[2] = {0, 0};

// ============================================================
//  SENSOR BIORDINARIO (API del enunciado, todo en este .ino)
// ============================================================
...
int* leerDatosOrdinarios() {
  datosOrdinarios[0] = 0;
  datosOrdinarios[1] = (int)'?';

  if (!comprobardatosDisponibles()) {
    return datosOrdinarios;
  }
  ...
  if (sep > 0 && sep < (int)linea.length() - 1) {
    datosOrdinarios[0] = linea.substring(0, sep).toInt();
    datosOrdinarios[1] = (int)linea.charAt(sep + 1);
  } else if (linea.length() >= 2) {
    datosOrdinarios[0] = linea.substring(0, linea.length() - 1).toInt();
    datosOrdinarios[1] = (int)linea.charAt(linea.length() - 1);
  }
...
// ============================================================
//  FUNCION: DatabaseInsert — sensor biordinario
//  Inserta valor numérico + carácter alfanumérico en BD.
// ============================================================
void DatabaseInsert(int valorNumerico, char valorAlfanumerico) {
  if (!conectarDB()) return;

  char sql[280];
  snprintf(sql, sizeof(sql),
    "INSERT INTO `smart_residencial`.`Sensores_Biordinarios` "
    "(`ValorNumerico`,`ValorAlfanumerico`,`Fecha`,`Activo`) "
    "VALUES (%d,'%c',NOW(),1);",
    valorNumerico, valorAlfanumerico);
  ejecutarQuery(sql);

  Serial.print("  [BIORDINARIO] Insertado: ");
  Serial.print(valorNumerico);
  Serial.print(" / '");
  Serial.print(valorAlfanumerico);
  Serial.println("'");
}
...
void setup() {
  ...
  inicializarSensor(PIN_SENSOR_RX, PIN_SENSOR_TX);
  Serial.println("  Sensor biordinario (Serial2) inicializado.");
...
void loop() {
  ...
  // ── 3b. SENSOR BIORDINARIO (lectura por evento UART) ────
  if (comprobardatosDisponibles()) {
    int* datos = leerDatosOrdinarios();
    DatabaseInsert(datos[0], (char)datos[1]);
  }
