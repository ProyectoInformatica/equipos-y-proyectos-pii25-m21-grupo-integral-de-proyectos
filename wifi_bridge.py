...
def _store_telemetry(data: dict) -> int:
    ...
    # Pulsómetro: sus tres valores forman una sola lectura, así que se guardan
    # juntos en la tabla Pulsometro (no en la genérica Sensores). Solo se inserta
    # si el ESP32 envía al menos la frecuencia cardíaca.
    fc = _parse_float(data, "frecuencia_cardiaca")
    if fc is not None:
        sistolica  = _parse_float(data, "presion_sistolica")
        diastolica = _parse_float(data, "presion_diastolica")
        add_pulsometro_data(
            int(fc),
            int(sistolica) if sistolica is not None else 0,
            int(diastolica) if diastolica is not None else 0,
        )
        inserted += 1
